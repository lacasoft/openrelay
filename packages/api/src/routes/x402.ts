...
import { z } from 'zod'
import { type ChainClient, createChainClient, verifyUsdcTransfer } from '../lib/chain'
...
export async function x402Route(app: FastifyInstance) {
   ...
   */
  app.post<{ Body: z.infer<typeof VerifySchema> }>('/x402/verify', async (req, reply) => {
    ...
    try {
          ...
          return reply
            ...
            .send(
              apiError(
                ...
                verification.reason ?? 'On-chain verification failed',
              ),
            )
        }
      }
        ...
        return reply
          ...
          .send(
            apiError(
              ...
              `Required ${amount} but received ${amountReceived}.`,
              'amount',
            ),
          )
      }
      ...
      return reply
        .status(400)
        .send(apiError('invalid_payment_payload', 'Could not decode or parse the payment payload.'))
    }
